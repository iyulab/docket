...
2. **Check what you already hold**: `list_items(mine=<your id>)` — catches an item claimed in a
   ...
   resolved` separately and merging them yourself. `mine` deliberately excludes `closed` items — once
   closed, nobody's turn ([ADR-0010](decisions/ADR-0010-item-from-to-turn.md)), so it's no longer
   something you "hold". That also means `mine` does not surface an item that reached `closed`
   without your involvement as requester — the four admin overrides (`remove`/`merge`/`force-close`/
   ...
   non-terminal and terminal state.
