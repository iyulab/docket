...
| `add_comment` | `item_id`, `body`, `author?` | `POST /items/{id}/comments {"author","body"}` | `author` defaults to `"unknown"` if omitted. **Never changes `state` or `turn`** — a fully-narrated workflow told entirely through comments leaves the item exactly where `claim_item`/`submit_item`/etc. last left it |
