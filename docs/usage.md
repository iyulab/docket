...
|---|---|---|---|
| `register_worker` | `id?`, `topics[]` | `POST /workers` | Call once per session. `topics` are prefixes — see §5. `id` **resolvable-required** at this tool layer (cycle-58, same pattern as `claim_item` above) — omit it to use this session's `DOCKET_WORKER_ID`; a tool-level error only if neither is present. Two drift cases behave differently and the difference matters: an `id` differing only in **case** lands on the existing row and the response returns its canonical spelling (adopt it — [ADR-0021](decisions/ADR-0021-case-insensitive-identity.md) chose that over a `409`), but a **bare `id` with no org scope** (`widget` where the server knows `acme/widget`) is a *different* identity, so the upsert cannot fold it and a second worker row appears. That one can't be resolved automatically, so the tool result carries an `id_advisory` naming the scoped spellings it collides with — the same bare-leaf gate `create_item`'s `requester_advisory` uses. Registration succeeds either way; the advisory never blocks it. A worker whose spelling split this way has a *different jurisdiction* than the one holding the work, so its `mine`/`topic_scope` silently go quiet |
...
| `create_item` | `topic`, `title`, `body?`, `tags[]?`, `requester?` | `POST /items` | **Call `search_items` first** to avoid filing a duplicate. `requester` is who this item is being worked for — optional (see [ADR-0010](decisions/ADR-0010-item-from-to-turn.md) / [ADR-0011](decisions/ADR-0011-requester-assignee-naming.md)). Attaches up to three advisory strings to the tool result, none of which blocks the create. `topic_advisory` — no registered worker serves this topic (see `GET /topics/candidates` below). `topic_spelling_advisory` / `requester_advisory` — the `topic`/`requester` you passed is a **bare name with no org scope** and the server already knows scoped identities with the same name, the shape a short-form spelling drift takes *as it is created*. Deliberately not plain segment equality: two scoped identities sharing a leaf (`acme/booster`, `other-org/booster`) are routinely unrelated and would warn on every filing forever, so the bare-leaf direction is the gate. **The two topic advisories are separate fields and can both fire**, because they fail for different reasons — "nobody serves this" and "this collides with a scoped spelling" are independent, and a bare topic that a worker happens to be registered for passes the first while failing the second. See [ADR-0025](decisions/ADR-0025-identity-enumeration-and-drift.md)'s 2026-09-22 update |
