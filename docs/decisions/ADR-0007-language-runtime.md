...
**Gained**: all three layers (core/cc/mcp) ship as pure native binaries, matching the installation-friction goal noted above ("a single install command"). The toolchain unifies on one (Cargo), which effectively narrows the monorepo-tooling question down to a Cargo workspace — though the "dependency-checking tool" itself (see [architecture.md](../architecture.md)'s "Single repo, enforced by mechanism," item 1) still needs to be chosen.
