...
**Status**: `docket-mcp` done (register/create/list/claim/submit/approve as MCP tools, an HTTP client of `docket-core` — see README "As an MCP server"). `docket-cc` scaffolding done: file representation + identifier mapping (`sync`, layout per [ADR-0008](decisions/ADR-0008-file-representation-location.md)), the `SessionStart` hook endpoint (`hook`, plain-text summary of open items), and local topic derivation (`topic` — the `org/repo` a directory belongs to, derived from its nearest `.git` remote, monorepo/submodule/worktree-aware) all exist — see README "As a local file projection" / "As a Claude Code hook" / "As a topic-deriving helper". No local daemon behind `hook` yet; each invocation does its own one-shot sync, and whether a daemon is actually needed is still open. What's left for M2's own completion criterion isn't more code — it's actually running two live sessions handing an item back and forth end to end, which is a human dogfooding step no cycle can simulate on its own.
