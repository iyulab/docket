...
/// Sent on every GitHub API/asset request this crate makes — GitHub rejects
/// requests with no `User-Agent` header, and one shared string is enough
/// since no code path here branches on which worker (`docket-mcp`,
/// `docket-cc`, ...) is being resolved.
pub(crate) const USER_AGENT: &str = "docket-launcher";
